 ...
 * and their costs that make up the MST as well as the total weight cost.
 * 
 * @author David Kim
 * @author Tyler Powers
 * @author Mahir Ahmed
...
public class Kruskals {
	...
	/**
	 * Runs Kruskal's algorithm on graph G and prints out the edges and their
	 * weights that make up the MST as well as the MST total weight cost.
	 * 
	 * @param nodes
	 *            an array of all the edges in the graph to be used in the
	 *            BinaryHeap
	 */
	public static void kruskal(SimpleGraph G) {
		...
		// An array of EdgeHeapNodes the size of the number of edges in the
		// graph
		...
		// Assign each vertex a number between 0 and |V|-1 to be used with the
		// disjoint set
		...
		for (k = G.edges(); k.hasNext();) {
			...
			e = (Edge) k.next();
			nodes[counter] = new EdgeHeapNode((double) e.getData(),
					e.getFirstEndpoint(), e.getSecondEndpoint());
			counter++;
		...
		// while the graph doesn't include all the vertices check if each edge
		// is creates
	...
	/**
	 * Create a graph load vertices and edges from a file and run Kruskal's
	 * algorithm on it.
	 * 
	 * @param args
	 */
	public static void main(String[] args) {
		...
		/*
		 * System.out.println(); System.out.println("Number of vertices = " +
		 * NUM_VERTICES); System.out.println("Number of edges = " +
		 * G.numEdges()); System.out.println();
		 */
