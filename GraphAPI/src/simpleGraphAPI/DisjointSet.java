...
/**
 * Implements equivalence relations as an array where the item in the array at
 * an index indicates which equivalence relation it belongs to. Negative number
 * represents it being the root and negative numbers indicate size.
 * 
 * 
 * @author Tyler Powers
 * @author David Kim
...
public class DisjointSet {
	...
	/**
	 * Constructor for DisjointSet class initializing all uptree's to be in
	 * their own set.
	 * 
	 * @param size
	...
	/**
	 * Make the bigger uptree of v1 and v2 the root of the other and update the
	 * size. If they are the same size make v1 the root of v2. If the uptrees of
	 * v1 and v2 are in the same equivalence relation no union is done.
	 * 
	 * @param v1
	 *            the first uptree being unioned
	 * @param v2
	 *            the second uptree being unioned
	 */
	public void union(int v1, int v2) {
		...
		// size of uptree at v1 is bigger than or equal to the size of the
		// uptree at v2
		else if (uptree[v1] <= uptree[v2]) {
			uptree[v1] = uptree[v1] + uptree[v2]; // add the size of v2 to the
													// size of v1
			uptree[v2] = v1; // set the root of uptree at v2 to be v1
		}
		// uptree at v2 is a bigger tree than uptree at v1
		else if (uptree[v1] > uptree[v2]) {
			uptree[v2] = uptree[v2] + uptree[v1]; // add the size of v1 to the
													// size of v2
	 ...
	 * 
	 * @param v1
	 *            the uptree whose root needs to be found
	...
	public static void main(String[] args) {
		...
		System.out
				.println("check the data in the uptree before second union(2,3)");
		...
		System.out
				.println("Check the data in the uptree after second union(2,3)");
		System.out.println("uptree[2]: " + disJoint.uptree[2]);
		System.out.println("uptree[3]: " + disJoint.uptree[3]);
		System.out.println();
		...
		System.out.println();
		System.out
				.println("expect find of 1 to be 2 since 1 points to 0 which points to 2");
